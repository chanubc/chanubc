## 👋 Introduction

**"1,500명 유저 서비스를 운영한 엔지니어링 역량에 AI 자동화를 더해, 실질적인 가치를 만드는 개발자 배찬우입니다."**

안드로이드 리드 개발자로서 서비스를 런칭하고 운영하며 **토큰 재발급 속도 200ms 개선, 이미지 전송 시간 300ms 단축** 등 구체적인 성능 최적화를 이뤄냈습니다. 현재는 이러한 탄탄한 개발 역량을 바탕으로, **n8n과 LLM을 활용한 비즈니스 자동화 솔루션**을 구축하며 기술의 영역을 **'모바일 클라이언트'에서 'AI 에이전트 서비스'로 확장**하고 있습니다.

* **Engineering Core:** 대규모 유저 트래픽 경험과 **Clean Architecture** 설계를 통한 안정적인 시스템 구축
* **Tech Expansion:** 기존 개발 역량에 **n8n & AI Agent** 기술을 접목하여, 기획부터 서비스 배포까지 주도하는 올라운더(All-Rounder) 지향

<br>

## 💼 Career

| 기간 | 소속 | 내용 |
| :--- | :--- | :--- |
| **2023.12 ~ 진행중** | 팀 예스비 | 한양대학교 산하 창업지원팀 Android Developer |
| **2024.03 ~ 2024.07** | NOW SOPT | 국내 최대 규모 대학생 연합 IT 벤처 창업 동아리 Android 파트 34기 OB |
| **2023.09 ~ 2024.01** | DO SOPT | 국내 최대 규모 대학생 연합 IT 벤처 창업 동아리 Android 파트 33기 YB |
| **2023.03 ~ 2023.12** | 멋쟁이사자 | 연합 IT 동아리 멋쟁이사자 11기 Frontend |
| **2022.09 ~ 2023.02** | UMC | 연합 IT 동아리 UMC 3기 Android |
| **2022.12 ~ 2023.02** | (주)Ezpmp | 플랫폼 비즈니스 운영 인턴 (Software 개발자 현장실습) |
| **2021.03 ~ 2025.08** | 건국대학교 | 스마트ICT융합공학과 전공 / 문화콘텐츠학과 부전공 |

<br>

## 🏆 Awards

| 일시 | 대회 및 활동 | 수상 결과 |
| :--- | :--- | :--- |
| **2024.08** | 2024 중소벤처기업부 창업 경진대회 | 🥇 **최우수상 (창업진흥원장상)** |
| **2024.07** | 연합 IT 동아리 SOPT 34기 장기 해커톤 App Jam | 🥇 **최우수상** |
| **2024.01** | 연합 IT 동아리 SOPT 33기 장기 해커톤 App Jam | 🥇 **최우수상** |
| **2023.08** | 한국보건산업진흥원 공공데이터 활용 공모전 | 🏆 **대상 (한국보건산업진흥원장상)** |
| **2022.02** | 실감콘텐츠 기획 공모전 | 🥇 **최우수상 (인문콘텐츠학회장상)** |

> **Project Highlight**
> * **[대상] 한국보건산업진흥원 공공데이터 활용 공모전:** 광자 인식(OCR) 및 공공데이터 API를 활용한 '청소년 영양 솔루션 앱' 개발 [[관련기사](https://it.chosun.com/news/articleView.html?idxno=2023090101142)]

<br>

## 💻 Projects

| 기간 | 이름 | 내용 | 파트 |
| :--- | :--- | :--- | :--- |
| **2026.01** | [나락분석봇](https://pf.kakao.com/_xbHnxon) | 카카오톡 채팅을 활용한 유튜브 댓글 여론 분석 챗봇 | 기획 · AI Agent |
| **24.08 ~ 진행중** | [Wable](https://github.com/Team-Wable/WABLE-ANDROID) | Clean Lck를 위한 온화한 익명 커뮤니티 | Android |
| **24.07 ~ 24.10** | [꾸물꿈](https://github.com/OMZigak/KKUM_ANDROID) | 친구들과 재미있고 확실한 지각 문제를 해결해 주는 서비스 | Android 리드 |
| **2025.01** | [Photos-LruCache](https://github.com/chanubc/Photos-LruCache) | 이미지 라이브러리 없이 구현한 LRU Cache 기반 갤러리 앱 | Android |
| **23.12 ~ 24.06** | [Don’t Be](https://github.com/TeamDon-tBe/ANDROID) | 투명도를 활용한 온화한 IT메이커들을 위한 커뮤니티 | Android |
| **23.06 ~ 23.07** | [Lite-Weight](https://github.com/Lite-Weight/LiteWeight_Android) | OCR 및 공공데이터 API를 활용한 청소년 영양 솔루션 앱 | 기획 · 디자인 · Android |
| **23.07 ~ 23.08** | [농토리](https://github.com/chanubc/Nongtori) | 농수산물 직거래 플랫폼 서비스 | Frontend |

<br>

## 🛠 Tech Stacks

| Category | Stacks |
| :--- | :--- |
| **AI & Automation** | n8n (Workflow), AI Agent Dev, OpenAI API, Claude, Solar |
| **Languages** | Kotlin, SQL, JavaScript, HTML, CSS |
| **Frameworks** | Android Jetpack, Jetpack Compose, React |
| **Architecture** | Clean Architecture, MVVM, MVI |
| **Libraries** | Hilt, Retrofit2, OkHttp, Coroutines, Flow, Room |
| **Infra & Tools** | GCP, Docker, PostgreSQL, Nginx |
